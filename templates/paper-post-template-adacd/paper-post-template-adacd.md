...
description: "安全对齐后的 LLM 常对无害问题过度拒绝，AdaCD 通过自适应对比解码，在不需要微调的情况下同时降低过度拒绝率并保持模型安全性。"
date: 2026-04-07
tags: ["LLM Safety", "Contrastive Decoding", "Over-Refusal"]
...
{{< figure src="workflow.png" alt="AdaCD 整体框架图" caption="图 1. AdaCD 的整体框架，包括拒答 token 分布提取与自适应解码模式切换两个模块。" >}}
...
{{< alert "lightbulb" >}}
**核心结论**：AdaCD证明了LLM在下一token预测时，通过一致性比率和自适应置信约束动态切换解码模式，可以用极低的推理开销有效解决过度拒绝与模型安全性保持这一矛盾。
{{< /alert >}}
